...
function getNextFriday(date) {
  ...
  if (dateNow.getUTCDay() === 5) {
    dateNow.setUTCDate(dateNow.getUTCDate() + 7);
  } else {
    while (dateNow.getUTCDay() !== 5) {
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const InDays = ms / (1000 * 60 * 60 * 24) + 1;
...
function formatDate(date) {
  ...
  const month = inDate.getUTCMonth() + 1;
  const day = inDate.getUTCDate();
  const year = inDate.getUTCFullYear();

  let hours = inDate.getUTCHours() % 12;
  hours = hours === 0 ? 12 : hours;
  const minutes = inDate.getUTCMinutes();
  const seconds = inDate.getUTCSeconds();

  const ampm = inDate.getUTCHours() >= 12 ? 'PM' : 'AM';
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = date.getDate(); day <= lastDay.getDate(); day += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const dayNumber = Math.floor((d - yearStart) / (24 * 60 * 60 * 1000)) + 1;
